...
cuteSVTrio_1kgp_same_population_trio_member_ls = ["HG00597","HG00595","HG00596","HG01172","HG01170",
                                                  "HG01171","HG02321","HG02281","HG02282","HG02770",
                                                  "HG02768","HG02769","HG03122","HG03120","HG03121",
                                                  "HG03584","HG03582","HG03583","HG03134","HG03133",
                                                  "HG03132","NA10831","NA12155","NA10863","NA12264",
                                                  "NA19145","NA19143","NA20358","NA20357","NA20279","NA20278"]
...
cuteSVTrio_HGSVC_member_ls = ["HG00512","HG00513","HG00514","HG00731","HG00732","HG00733",
                              "HG01890","HG02666","HG03371","HG03452","HG03520","NA19238",
                              "NA19239","NA19240","NA19705","NA19836"]

# 这个样本列表是从1kGP/1kGP_chr1_SV.15.target.vcf.gz中随机提取出来的
...
# 从HGSVC中挑选样本，保证和cuteSVTrio中加入的样本人种一致，同时和现有样本不成家庭关系
def random_comple_samples_2HGSVC() :
    pick_HGSVC_member_ls = cuteSVTrio_HGSVC_member_ls
    onekGP_vcf = "1kGP/1kGP_chrall_SV.vcf.gz"
    fh = gzip.open(onekGP_vcf, 'rt') if onekGP_vcf.endswith('.gz') else open(onekGP_vcf, 'rt')
    for line in fh:
        if line == "" :
            continue
        line = line.strip()
        if line.startswith('#CHROM'):
            onekGP_sample_ls = line.strip().split("\t")[9:]
            break
    
    HGSVC_vcf = "freeze4/merge.truvari.rectified.vcf.gz"
    fh = gzip.open(HGSVC_vcf, 'rt') if HGSVC_vcf.endswith('.gz') else open(HGSVC_vcf, 'rt')
    for line in fh:
        if line == "" :
            continue
        line = line.strip()
        if line.startswith('#CHROM'):
            HGSVC_sample_ls = line.strip().split("\t")[9:]
            break

    onekGP_sample_relatives = [[],[],[],[]]
    with open("1kGP/1kGP.3202_samples.pedigree_info_population.txt",'r') as f :
        while(True) :
            f_line = f.readline().strip()
            if f_line == "" :
                break
            f_split_ls = f_line.split(" ")
            onekGP_sample_relatives[0].append(f_split_ls[0])
            onekGP_sample_relatives[1].append(f_split_ls[1])
            onekGP_sample_relatives[2].append(f_split_ls[2])
            onekGP_sample_relatives[3].append(f_split_ls[4])
        f.close()

    ralative_member_ls = []
    for sample in cuteSVTrio_HGSVC_member_ls :
        ralative_member_ls.append(sample)
        if sample in onekGP_sample_relatives[0] :
            sample_index = onekGP_sample_relatives[0].index(sample)
            if onekGP_sample_relatives[1][sample_index] != "0" and onekGP_sample_relatives[1][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
            if onekGP_sample_relatives[2][sample_index] != "0" and onekGP_sample_relatives[2][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
        if sample in onekGP_sample_relatives[1] :
            sample_index = onekGP_sample_relatives[1].index(sample)
            if onekGP_sample_relatives[0][sample_index] != "0" and onekGP_sample_relatives[0][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
            if onekGP_sample_relatives[2][sample_index] != "0" and onekGP_sample_relatives[2][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
        if sample in onekGP_sample_relatives[2] :
            sample_index = onekGP_sample_relatives[2].index(sample)
            if onekGP_sample_relatives[0][sample_index] != "0" and onekGP_sample_relatives[0][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
            if onekGP_sample_relatives[1][sample_index] != "0" and onekGP_sample_relatives[1][sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
    print(ralative_member_ls)
    for sample in cuteSVTrio_1kgp_member_ls :
        print(sample)
        if sample in pick_HGSVC_member_ls :
            continue
        sample_population = onekGP_sample_relatives[3][onekGP_sample_relatives[0].index(sample)]
        while(True) :
            random_sample = random.choice(HGSVC_sample_ls)
            if random_sample in pick_HGSVC_member_ls or random_sample in ralative_member_ls or random_sample not in onekGP_sample_ls:
                continue
            random_sample_index = onekGP_sample_relatives[0].index(random_sample)
            if onekGP_sample_relatives[3][random_sample_index] != sample_population :
                continue
            pick_HGSVC_member_ls.append(random_sample)
            if onekGP_sample_relatives[1][random_sample_index] != "0" and onekGP_sample_relatives[1][random_sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][random_sample_index])
            if onekGP_sample_relatives[2][random_sample_index] != "0" and onekGP_sample_relatives[2][random_sample_index] not in ralative_member_ls:
                ralative_member_ls.append(onekGP_sample_relatives[1][random_sample_index])
            
            if random_sample in onekGP_sample_relatives[1] :
                random_sample_index = onekGP_sample_relatives[1].index(random_sample)
                if onekGP_sample_relatives[0][random_sample_index] != "0" and onekGP_sample_relatives[0][random_sample_index] not in ralative_member_ls:
                    ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
                if onekGP_sample_relatives[2][random_sample_index] != "0" and onekGP_sample_relatives[2][random_sample_index] not in ralative_member_ls:
                    ralative_member_ls.append(onekGP_sample_relatives[1][sample_index])
            if random_sample in onekGP_sample_relatives[2] :
                random_sample_index = onekGP_sample_relatives[2].index(random_sample)
                if onekGP_sample_relatives[0][random_sample_index] != "0" and onekGP_sample_relatives[0][random_sample_index] not in ralative_member_ls:
                    ralative_member_ls.append(onekGP_sample_relatives[1][random_sample_index])
                if onekGP_sample_relatives[1][random_sample_index] != "0" and onekGP_sample_relatives[1][random_sample_index] not in ralative_member_ls:
                    ralative_member_ls.append(onekGP_sample_relatives[1][random_sample_index])
    ...
    print(pick_HGSVC_member_ls)

# 按照cuteSV-Trio与1kGP，HGSVC与1kGP的样本的相交关系，从三者中提取对应的样本，作为整合为panel的前置数据集，并且只保留ins和del
def pick_samples_from_vcf(read_vcf,write_vcf,sample_ls,relative_sample_ls) :
    sv_type_ls = ["INV","DUP"]
    sample_index = []
        ...
        if line.startswith('#'):
            if line.startswith('#CHROM'):
                ...
                for sample_name in sample_ls :
                    ...
                    new_line_split_ls.append(relative_sample_ls[sample_ls.index(sample_name)])
                new_lines.append("\t".join(new_line_split_ls))
                print(sample_ls)
                print(new_line_split_ls[9:])
                #if len(sample_index) != len(sample_ls) :
                #    print(sample_ls)
                #    print(new_line_split_ls[9:])
                #    return
            else :
            ...
            line_split_ls = line.split("\t")
            sv_type_index = -1
            for sv_i in range(len(sv_type_ls)) :
                if sv_type_ls[sv_i] in line_split_ls[7] :
                    sv_type_index = sv_i
            if sv_type_index != -1 :
                continue
            new_line_split_ls = line_split_ls[0:9]
...
def main(argv):
    pick_samples_from_vcf("cutesv_trio/merge.truvari.rectified.chr1.vcf.gz".replace("chr1",argv[0]),"cutesv_trio/cuteSVTrio.chr1.overlap.sv.vcf".replace("chr1",argv[0]),cuteSVTrio_1kgp_member_ls,cuteSVTrio_1kgp_member_ls)
    pick_samples_from_vcf("1kGP/1kGP_chr1_SNP_INDEL.vcf.gz".replace("chr1",argv[0]),"cutesv_trio/1kGP.chr1.overlap.SNP_INDEL.vcf".replace("chr1",argv[0]),cuteSVTrio_1kgp_member_ls,cuteSVTrio_1kgp_member_ls)
    pick_samples_from_vcf("freeze4/merge.truvari.rectified.chr1.vcf.gz".replace("chr1",argv[0]),"freeze4/samples_31/HGSVC.chr1.overlap.sv.vcf".replace("chr1",argv[0]),HGSVC_1kgp_member_ls,HGSVC_1kgp_member_ls)
    pick_samples_from_vcf("1kGP/1kGP_chr1_SNP_INDEL.vcf.gz".replace("chr1",argv[0]),"freeze4/samples_31/1kGP.chr1.overlap.SNP_INDEL.vcf".replace("chr1",argv[0]),HGSVC_1kgp_member_ls,HGSVC_1kgp_member_ls)
